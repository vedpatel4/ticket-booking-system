...
export class TicketService {
  async getAvailableTickets(): Promise<ITicket[]> {
    ...
    return tickets.map(
      (ticket) =>
        new Ticket(ticket.ticketId, ticket.type as TicketType, ticket.price),
    );
  }
  ...
  async getTicketById(ticketId: string): Promise<Ticket | null> {
    ...
    return ticket ? new Ticket(ticket.ticketId, ticket.type as TicketType, ticket.price) : null;
  }
  ...
  private async createTicketsByType(
    ...
    for (let row = startRow; row < startRow + numRows; row++) {
      for (let col = 1; col <= columns; col++) {
        ...
        tickets.push(
          new Ticket(ticket.ticketId, ticket.type as TicketType, price),
        );
      }
    }
